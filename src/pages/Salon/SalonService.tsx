...
const SalonService: React.FC = () => {
  ...
  const fetchServiceSlots = async () => {
    try {
      ...
      if (res.data) setsalonserviceslotsdata(res.data);
    } catch (error) { console.error(error); }
  };

  const formatToShortMonthDay = (dateString: string) => {
  ...
  const BookAppointment = async (e: any) => {
    ...
    } finally { setbookingrequestsend(false); }
  };
        ...
        <section className="relative h-[50vh] lg:h-screen lg:sticky lg:top-0 overflow-hidden bg-slate-100">
          ...
          >
            {(salonservicedata?.saloninfoDTO?.images?.length > 0
              ...
              <SwiperSlide key={i}>
                <img src={img} alt="Service Detail" className="w-full h-full object-cover" />
              </SwiperSlide>
            ))}
          </Swiper>
          ...
          <div className="max-w-xl mx-auto space-y-12">
            ...
            <div className="space-y-6">
              <h1 className="text-4xl lg:text-6xl font-light tracking-tight text-balance leading-tight">
                {salonservicedata?.saloninfoDTO?.heading || "Luxury Experience"}
              </h1>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {salonservicedata?.descriptions?.map((desc: any, i: number) => (
                  <p key={i} className="text-lg text-slate-600 font-light leading-relaxed">
                    {desc}
                  </p>
                ))}
              </div>
            ...
            <div className="bg-slate-50 p-8 rounded-none border-l-4 border-slate-900">
              ...
              <ul className="space-y-4">
                {salonservicedata?.saloninfoDTO?.included_items?.map((item: any, i: number) => (
          ...
          <Card className="relative w-full max-w-2xl bg-white rounded-t-3xl lg:rounded-none border-none shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-500">
            ...
            <CardContent className="p-8 space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                  {uniqueSortedDates?.map((item, index) => (
                    <button
                      key={index}
                      onClick={() => { setSelectedDate(item); setselectedslottime(""); }}
                      className={`flex flex-col items-center justify-center min-w-[70px] h-20 transition-all border
                        ${selectedDate === item ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-100 text-slate-400 hover:border-slate-300"}`}
                    >
              ...
              {selectedDate ? (
                    ...
                    .map((slot, index) => (
                      <button
                        ...
                        className={`py-4 text-xs font-bold uppercase tracking-widest transition-all border
                          ${selectedslottime === slot.time ? "bg-[#1E4D8C] border-[#1E4D8C] text-white shadow-lg" : "bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100"}`}
                      >
                ...
                <div className="py-20 text-center text-slate-300 text-sm font-light italic">
                  Please select a date above to view availability.
                </div>
              )}
